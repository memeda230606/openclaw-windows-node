...
- OSS manifest 不可用时默认回退官方 URL 或 `wsl.exe --install --web-download`；可通过 `AllowOfficialFallback=false` 改为直接失败。
...
2. 增加 Microsoft WSL MSI 离线安装策略，减少缺少 WSL 平台时对 Microsoft Store/GitHub 的依赖。
3. 为 Setup Engine 控制台输出、失败提示、向导动态内容建立中文资源文件。
4. 增加“国内网络模式”预检，提前检查 OSS manifest、WSL、端口、权限和回退策略。
